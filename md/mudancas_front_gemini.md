...
* **Mudanças:**
  ...
  * **Tratamento de Notas Estruturadas (Fix Gemini):** Corrigimos o tipo de dados `notas` do catálogo `PerfumeExpandido` para aceitar tanto `string[]` quanto objetos com a estrutura `{ topo?: string[]; coracao?: string[]; fundo?: string[] }`. Implementamos o mapeamento seguro usando `Array.isArray()` tanto no método `addExpandido` (linha 542) quanto na lógica de busca por compatibilidade de fallback (linha 834), juntando todas as notas antes de realizar o `.slice()`.
